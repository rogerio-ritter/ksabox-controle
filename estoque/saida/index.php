...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Saída de Estoque</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Registro de saídas, consumo e baixas de produtos</p>
    </div>
    <button onclick="openModal()"
        class="flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition-colors">
        <i class="fas fa-arrow-up mr-1"></i> Nova Saída
    </button>
</div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 class="font-semibold text-gray-800 dark:text-white flex items-center gap-2">
                <i class="fas fa-arrow-up text-orange-500"></i> Nova Saída de Estoque
            </h3>
        ...
        <div class="p-6 space-y-4">
            ...
            <div>
                ...
                <div id="saldo-hint" class="hidden mt-1.5 flex items-center gap-2 text-xs">
                    <span class="text-gray-500 dark:text-gray-400">Disponível:</span>
                    <span id="saldo-atual" class="font-semibold"></span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input id="f-quantidade" type="number" min="0.01" step="0.01" placeholder="0.00"
                        class="inp" oninput="validarQtd()">
                    <p id="qtd-warning" class="hidden text-xs text-red-500 mt-1">
                        <i class="fas fa-exclamation-triangle mr-1"></i>Quantidade maior que o saldo disponível.
                    </p>
                </div>
                <div>
            ...
            <div>
                <label class="lbl">Referência <span class="text-xs text-gray-400">(orçamento, NF, etc.)</span></label>
                <input id="f-referencia" type="text" placeholder="Ex: ORC-20250101-001" class="inp">
            </div>
        ...
        <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            ...
            <button id="btn-save" onclick="saveForm()"
                class="px-4 py-2 text-sm bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-lg transition-colors">
...
async function carregaSelects() {
    ...
    sel.innerHTML = '<option value="">Selecione...</option>' +
        produtosData.map(p => {
            ...
            const semSaldo = saldo <= 0 ? ' (sem saldo)' : '';
            return `<option value="${p.id}" data-saldo="${p.saldo}" data-unidade="${esc(p.unidade_sigla)}">${esc(p.nome)}${semSaldo}</option>`;
        }).join('');
}
...
function onProdutoChange() {
    ...
    if (opt?.value) {
        ...
        el.textContent = saldo.toFixed(2) + ' ' + unid;
        el.className = saldo > 0 ? 'font-semibold text-green-600 dark:text-green-400'
...
function validarQtd() {
    ...
    const warn = document.getElementById('qtd-warning');
    warn.classList.toggle('hidden', qtd <= saldoAtualNum || qtd <= 0);
}
...
function loadData() {
    ...
    searchTimeout = setTimeout(async () => {
        ...
        try {
            ...
            if (!data.length) {
                tbody.innerHTML = '<tr><td colspan="7" class="px-4 py-10 text-center text-gray-400 dark:text-gray-500">Nenhuma saída encontrada.</td></tr>';
                return;
            }
            tbody.innerHTML = data.map(r => `
                <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-4 py-3 text-right">
                        <span class="font-semibold text-orange-600 dark:text-orange-400">-${parseFloat(r.quantidade).toFixed(2)}</span>
                    ...
                    <td class="px-4 py-3 text-right">
                        <button onclick="cancelar(${r.id}, '${esc(r.produto_nome)}', ${r.quantidade})"
                            class="text-red-400 hover:text-red-600" title="Cancelar saída">
...
function openModal() {
    ...
    document.getElementById('qtd-warning').classList.add('hidden');
    saldoAtualNum = 0;
...
function cancelar(id, nome, qtd) {
    confirmDialog('Cancelar Saída',
        `Cancelar a saída de ${parseFloat(qtd).toFixed(2)} de "${nome}"? A quantidade será devolvida ao estoque.`,
        async () => {
